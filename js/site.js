...
function buildSummary() {
    ...
    costResult.innerHTML = `${formatNumber(finalCost)}`;
}
...
function downPayment() {
    let loanAmountD = principal;
    let depositAmount = parseInt(document.getElementById('downPayment').value) || 0;
    let downPaymentPercentage = parseFloat(document.getElementById('downPaymentPercentage').value) || 0.0;

    if (depositAmount == 0 && downPaymentPercentage == 0) {
        calculate();
    } else if (depositAmount == 0){
        document.getElementById('downPayment').disabled = true;
        document.getElementById('downPaymentPercentage').disabled = false;
    } else {
        document.getElementById('downPayment').disabled = false;
        document.getElementById('downPaymentPercentage').disabled = true;
    }
}